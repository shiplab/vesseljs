...
function Ocean(params) {
	...
	*/
	try {
		...
		THREE.Mesh.call(this, new THREE.PlaneBufferGeometry(this.size, this.size, this.segments, this.segments), /*new THREE.MeshPhongMaterial({
				color: 0x041020,
		...
		//Axes:
		this.add(new THREE.AxesHelper(600));
	}
	...
	if (params.parentGUI) {
		...
		this.cosConf.add(
			{
				next: function () {
					...
					while (i < 2 * scope.waves.length && scope.waves[i % scope.waves.length].waveType !== "Cosine") {
						i++;
					}
		...
		this.splashConf.add(
			{
				next: function () {
					...
					i = (i + 1) % scope.waves.length;
					while (i < 2 * scope.waves.length && scope.waves[i % scope.waves.length].waveType !== "Fake splash") {
						i++;
					}
					if (i === 2 * scope.waves.length) return;
		...
		this.sampledConf.add(
			{
				next: function () {
					...
					i = (i + 1) % scope.waves.length;
					while (i < 2 * scope.waves.length && scope.waves[i % scope.waves.length].waveType !== "From samples") {
						i++;
					}
					if (i === 2 * scope.waves.length) return;
		...
		this.sampledConf.add(psampled, "filename").onChange(function () {
			scope.sampledConf.updateDisplay();
		});
