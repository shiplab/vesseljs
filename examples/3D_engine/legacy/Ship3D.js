...
function Ship3D(ship, stlPath) {
	...
	let hMat = new THREE.ShaderMaterial({
		...
		fragmentShader: commonDecl + phong.fragmentShader.replace("vec4 diffuseColor = vec4( diffuse, opacity );", "vec4 diffuseColor = vec4( (vPos.z>wlThreshold)? aboveWL.rgb : belowWL.rgb, opacity );").replace("#define PHONG", ""),
	...
	addObject: function (object) {
		...
		if (this.materials[name] !== undefined) {
			...
			if (object.style.color !== undefined) {
				mat = new THREE.MeshPhongMaterial({ color: object.style.color, transparent: true, opacity: object.style.opacity })
			} else {
		...
		if (bo.file3D) {
			...
			this.stlLoader.load(
				...
				function onLoad(geometry) {
					...
					geometry.scale(1 / (b.max.x - b.min.x), 1 / (b.max.y - b.min.y), 1 / (b.max.z - b.min.z))
