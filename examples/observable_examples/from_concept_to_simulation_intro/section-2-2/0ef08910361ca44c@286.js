...
On this section we will continue to explore the general arrangement, defining the compartments inside the ship. So far, it was already defined Deck plates and bulkheads location.
`
)}

function _2(md){return(
md`
### <span style="color:rgb(13, 18, 125)"> Compartments Identification

Two variables defines the compartments in <code>Gunnerus.baseObjects</code> and <code>Gunnerus.derivedObjects</code>. The Base Objects have information regarding the compartment, for example, its lenghts, weights and cg. The Derived Objects is related to its relationship to the whole vessel such as the location.
`
)}
...
md`
Every base and derived object will have an identification (id.) that is linked to each other. The engine room is going to be used for the for exemplification about how each compartment is defined starting by definition of its ids:
`
)}

function _7(Gunnerus){return(
Gunnerus.derivedObjects[0].id = "Engine Room"
)}

function _8(Gunnerus){return(
Gunnerus.baseObjects[0].id = "engine_room.stl"
)}

function _9(Gunnerus){return(
Gunnerus.derivedObjects[0].baseObject = "engine_room.stl"
)}

function _10(md){return(
md`
### <span style="color:rgb(13, 18, 125)"> Compartments Dimensions and Location

The locations of the compartment are defined in the Derived Objects <code>Gunnerus.derivedObjects</code> in the position of reference state (<code>xCentre</code>, <code>yCentre</code>, <code>zBase</code>):
`
)}

function _11(Gunnerus){return(
Gunnerus.derivedObjects[0].referenceState = {
			"xCentre": 13.25,
			"yCentre": 0,
			"zBase": 2.385
		}
...
md`
The dimensions of the compartment are defined in the Base Objects <code>Gunnerus.baseObjects </code> in the position of reference state (<code>length</code>, <code>breadth</code>, <code>height</code>):
`
)}

function _13(Gunnerus){return(
Gunnerus.baseObjects[0].boxDimensions = {
			"length": 11.5,
			"breadth": 9.6,
			"height": 4.576
		}
...
md`
Now it is possible to create the 3D model of defined compartments:
`
)}
...
function* _18(THREE,width,ship,invalidation,ship3D)
{
...
md`
### <span style="color:rgb(13, 18, 125)"> Gunnerus Compartments

After the explanation about how to insert compartments into the model we can now define the compartments inside using our model ship Gunnerus. The necessary information about the sizes and location of each compartment is described in the table below:

<img src="https://raw.githubusercontent.com/ferrari212/Ship-Design/master/Figures/compartment.png" alt="drawing" width="80%"/>
`
)}
...
function* _23(THREE,width,ship,invalidation,ship3D2)
{
  const renderer = new THREE.WebGLRenderer({antialias: true});
...
md `
<p style="text-align: center;font-size: 25px;"> [<< Previous](../chapter-2/index.html) || <a href="#top">Top</a> || [Next >>](../section-2-3/index.html) </p> 
`
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["THREE","width","ship","invalidation","ship3D"], _18);
  ...
  main.variable(observer()).define(["THREE","width","ship","invalidation","ship3D2"], _23);
