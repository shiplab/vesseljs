...
var Wave = function() {
	...
	return function(waveType) {
		...
		this.waveType = waveType;
						...
						//A, T, theta, phi
function DirectionalCosine(params, per) {
	params = params || {};
	var oceanconf = params.parentGUI;
...
function Ocean(params) {
	...
	*/
	...
	if (params.parentGUI) {
		...
		this.conf.add(pcos, "A",0.0, 5, 0.5);
		var per = this.conf.add(pcos, "T",1,29, 1);
        
        /*per.onChange(function(activate){
            console.log('good');
        }); */
      //PER FER PROVES DE HEADING A:5 T:6
        

		this.conf.add(pcos, "theta", 0, 6, 0.25);
