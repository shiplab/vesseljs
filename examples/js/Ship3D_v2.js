...
function Ship3D(ship, {shipState, stlPath, upperColor, lowerColor, hullOpacity, deckOpacity=0.2, objectOpacity=0.5}) {
	...
	this.hull3D = new Hull3D(hull, upperColor, lowerColor, designDraft, hullOpacity);
...
function Hull3D(hull, upperColor=0x33aa33, lowerColor=0xaa3333, design_draft, opacity=0.5) {
	...
	let hMat = new THREE.ShaderMaterial({
		uniforms: THREE.UniformsUtils.merge([phong.uniforms, {
			wlThreshold: new THREE.Uniform(design_draft/Depth),
			aboveWL: new THREE.Uniform(new THREE.Color(upperColor)),
			belowWL:  new THREE.Uniform(new THREE.Color(lowerColor))	
		}]),
	...
	hMat.uniforms.opacity.value = opacity;
	...
	starboard.scale.y = -1;
	this.add(port, starboard);
	
	//Caps:
	this.add(new THREE.Mesh(bowCapG, hMat));
	this.add(new THREE.Mesh(aftCapG, hMat));
	this.add(new THREE.Mesh(bottomCapG, hMat));
